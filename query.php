...
if (isset($_POST['add'])){
	...
	if ($x) echo "Query successfully complete. <br />"; else die("Query fail: ".mysqli_error($conn)."<br />");
	//}
	mysqli_close($conn);
	}
?>
...
<table border="0" cellspacing="0" cellpadding="2">
	...
	<tr>
		...
		$x1 = mysqli_query($conn, "show tables");
		mysqli_close($conn);
...
if (isset($_POST['add'])){    //izvrsi ako postoji query
	echo "Prikaz ".mysqli_num_rows($x)." rezultata:";
	$fw=mysqli_num_fields($x);     //broj header fieldova
	?>
	...
	<?php
	$s=NULL;
	while ($row = mysqli_fetch_field($x)){
